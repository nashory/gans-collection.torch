...
local LSGAN = torch.class('LSGAN')


function LSGAN:__init(model, criterion, opt, optimstate)
    self.model = model
...
LSGAN['fDx'] = function(self, x)
    ...
    local errD_real = self.crit_adv:forward(f_x:cuda(), self.label:cuda())              -- minimize |D(x)-1|^2
    local d_real_crit = self.crit_adv:backward(f_x:cuda(), self.label:cuda())       
    ...
    -- forward and backward
    self.gen:forward(self.z:cuda())
    self.x_tilde = self.gen.output:clone()
    self.dis:forward(self.x_tilde:cuda())
    self.f_x_tilde = self.dis.output:clone()
    local errD_fake = self.crit_adv:forward(self.f_x_tilde:cuda(), self.label:cuda())   -- minimize |D(G(z))|^2
    if errD_fake>0.1 then
        local d_fake_crit = self.crit_adv:backward(self.f_x_tilde:cuda(), self.label:cuda())
        local d_fake_dis = self.dis:backward(self.x_tilde:cuda(), d_fake_crit:cuda())
    else
        local d_fake_crit = self.crit_adv:backward(self.f_x_tilde:cuda(), self.label:cuda())
        local d_fake_dis = self.dis:backward(self.x_tilde:cuda(), d_fake_crit:mul(0):cuda())
    end
...
LSGAN['fGx'] = function(self, x)
    self.gen:zeroGradParameters()

    self.label:fill(1)
    local errG = self.crit_adv:forward(self.f_x_tilde:cuda(), self.label:cuda())
    local d_gen_crit = self.crit_adv:backward(self.f_x_tilde:cuda(), self.label:cuda()) -- minimize |D(G(z))-1|^2
...
function LSGAN:train(epoch, loader)
    ...
    for e = 1, epoch do
        ...
        for iter  = 1, iter_per_epoch do
            ...
            self:snapshot(string.format('__6_lsgan/repo/%s', self.opt.name), self.opt.name, totalIter, data)
            ...
            if (totalIter%self.opt.display_iter==0) and (self.opt.display) then
                ...
                os.execute('mkdir -p __6_lsgan/repo/image')
                image.save(string.format('__6_lsgan/repo/image/%d.jpg', totalIter/self.opt.display_iter), im_png)
            end
...
function LSGAN:snapshot(path, fname, iter, data)
